...
int jerasure_matrix_decode(
    int k, int m, int w, int* matrix, int row_k_ones, int* erasures, char** data_ptrs, char** coding_ptrs, int size)
{
    int i, edd, lastdrive;
    int* tmpids;
...
void jerasure_bitmatrix_dotprod(
    int k, int w, int* bitmatrix_row, int* src_ids, int dest_id, char** data_ptrs, char** coding_ptrs, int size, int packetsize)
{
...
void jerasure_matrix_dotprod(
    int k, int w, int* matrix_row, int* src_ids, int dest_id, char** data_ptrs, char** coding_ptrs, int size)
{
    int init;
    char *dptr, *sptr;
    int i;
...
int jerasure_bitmatrix_decode(int k, int m, int w, int* bitmatrix, int row_k_ones, int* erasures, char** data_ptrs,
    ...
    for (i = 0; edd > 0 && i < lastdrive; i++) {
        if (erased[i]) {
            jerasure_bitmatrix_dotprod(
                k, w, decoding_matrix + i * k * w * w, dm_ids, i, data_ptrs, coding_ptrs, size, packetsize);
            edd--;
        }
    }
...
int jerasure_schedule_decode_lazy(
    int k, int m, int w, int* bitmatrix, int* erasures, char** data_ptrs, char** coding_ptrs, int size, int packetsize, int smart)
{
    int i, tdone;
    char** ptrs;
    int** schedule;
...
int jerasure_schedule_decode_cache(
    int k, int m, int w, int*** scache, int* erasures, char** data_ptrs, char** coding_ptrs, int size, int packetsize)
{
    int i, tdone;
    char** ptrs;
    int** schedule;
...
void jerasure_bitmatrix_encode(
    int k, int m, int w, int* bitmatrix, char** data_ptrs, char** coding_ptrs, int size, int packetsize)
{
    int i;
...
/*
void jerasure_update(int k, int m,int w,int *matrix,char **coding,int size,char *new_value, int index){
        char **new_coding;
        new_coding = talloc(char *, m);
        for (i = 0; i < m; i++)
                new_coding[i] = talloc(char, sizeof(long));

        char **new_data;
        long l =0;
        new_data = talloc(char *, k);
        for (i = 0; i <k; i++){
                new_data[i] = talloc(char, sizeof(long));
                memcpy(new_data[i], &l, sizeof(long));
        }
        memcpy(new_data[ndex], new_value, sizeof(long));
...
*/
// Bharath..had to write a function which will allow update of variables at position j
void jerasure_update(int k, int m, int w, int* matrix, char** coding_ptrs, int size, char* old_value, char* new_value, int j)
{
    ...
    if (dbg)
        printf("old data:");
    for (h = 0; h < size; h += (w / 8)) {
        if (dbg)
            printf(" ");
        for (x = 0; x < w / 8; x++) {
            if (dbg)
                printf("%02x", (unsigned char)old_value[h + x]);
            // printf("%d", data[i][j+x]);
        }
    }
    if (dbg)
    ...
    for (h = 0; h < size; h += (w / 8)) {
        ...
        for (x = 0; x < w / 8; x++) {
            if (dbg)
                printf("%02x", (unsigned char)new_value[h + x]);
            // printf("%d", data[i][j+x]);
        }
    }
    if (dbg)
    ...
    diff = talloc(char, sizeof(long));
    galois_region_xor(sptr, dptr, diff, size);
    ...
    for (h = 0; h < size; h += (w / 8)) {
        ...
        for (x = 0; x < w / 8; x++) {
            if (dbg)
                printf("%02x", (unsigned char)diff[h + x]);
        }
    }
    if (dbg)
        printf("\n");
    init = 0; // this option, given to galois multiply makes sure the answer of the mul is xored with whatever is in the dest.
    if (dbg)
        printf("matrix...:\n");
    if (dbg)
    ...
    for (i = 0; i < m; i++) {
        switch (w) {
        case 8:
            galois_w08_region_multiply(diff, matrix[i * k + j], size, dptr, init);
            break;
        case 16:
            galois_w16_region_multiply(diff, matrix[i * k + j], size, dptr, init);
            break;
        case 32:
            galois_w32_region_multiply(diff, matrix[i * k + j], size, dptr, init);
            break;
        }
        /*
        printf("after mul :");
        for(h=0;h< size; h+=(w/8)) {
                printf(" ");
                for(x=0;x < w/8;x++){
                        printf("%02x", (unsigned char)dptr[h+x]);
                        //printf("%d", data[i][j+x]);
                }
        }
        printf("\n");

        //dptr = mat[i,j]* (new_value-old_value)
        printf("Coding[%d] :",i);
        for(h=0;h< size; h+=(w/8)) {
                printf(" ");
                for(x=0;x < w/8;x++){
                        printf("%02x", (unsigned char)coding_ptrs[i][h+x]);
                        //printf("%d", data[i][j+x]);
                }
        }
        printf("\n");
        */
        galois_region_xor(coding_ptrs[i], dptr, coding_ptrs[i], size);
        /*
        printf("Final Coding[%d] :",i);
        for(h=0;h< size; h+=(w/8)) {
                printf(" ");
                for(x=0;x < w/8;x++){
                        printf("%02x", (unsigned char)coding_ptrs[i][h+x]);
...
// bharath: adds the content of the source_array to the dest_array
void jerasure_array_add(int array_size, char** dest_array, char** source_array, int size)
{
    int i = 0;
    for (i = 0; i < array_size; ++i)
        galois_region_xor(dest_array[i], source_array[i], dest_array[i], size);
}
// galois addition of two codes
void jerasure_code_add(char* code_dest, char* code_source, int size)
{
    galois_region_xor(code_dest, code_source, code_dest, size);
}

// bharath: given a code word, and updated data, finds the new code word.
void jerasure_update_single_code(
    int k, int m, int w, int mat_element, char* code, int i, int size, char* old_value, char* new_value, int j)
{
    ...
    if (dbg)
        printf("\n");
    init = 0; // this option, given to galois multiply makes sure the answer of the mul is xored with whatever is in the dest.
    //	  if(dbg)printf("matrix...:\n");
    //	  if(dbg)jerasure_print_matrix(matrix,m,k,w);

    // dptr = m_i_j * (d_j' - d_j)
    switch (w) {
    case 8:
        galois_w08_region_multiply(diff, mat_element, size, dptr, init);
        break;
    case 16:
        galois_w16_region_multiply(diff, mat_element, size, dptr, init);
        break;
    case 32:
        galois_w32_region_multiply(diff, mat_element, size, dptr, init);
        break;
    }

    // c_i' = c_i + dptr;
    galois_region_xor(code, dptr, code, size);
}

static void print_data(int w, int psize, char* data)
{
    int j, x;
    long l;
    for (j = 0; j < w; j++) {
...
void jerasure_cauchy_update(char* diff, char* coding_ptr, int* bitmatrix_element, int w, int psize, int size)
{
    // jerasure_print_bitmatrix(bitmatrix_element,w,w,w);
    char** data = talloc(char*, 1);
    data[0] = talloc(char, psize* w);
    memcpy(data[0], diff, psize * w * sizeof(char));
    // print_data(w,psize,data[0]);

    char** coding = talloc(char*, 1);
    coding[0] = talloc(char, psize* w);
    jerasure_bitmatrix_encode(1, 1, w, bitmatrix_element, data, coding, w * psize, psize);
    galois_region_xor(coding_ptr, coding[0], coding_ptr, psize * w);
    // print_data_and_coding(1,1, w, psize,data,coding);
}

// void jerasure_bitmatrix_update(char* data_diff,int packetsize, int w,char*
// coding_ptr,int* bitmatrix_element, int size ){
//   int j, sindex, pstarted, index, x, y;
//   char *dptr, *bdptr, *cptr, *bcptr;
//
//   if (size%(w*packetsize) != 0) {
//     fprintf(stderr, "jerasure_bitmatrix_dotprod - size%(w*packetsize)) must = 0\n");
//     exit(1);
...
//     for (j = 0; j < w; j++) {
//       pstarted = 0;
//       bdptr = data_diff; //pointer to block of data diff
//       bcptr = coding_ptr; //pointer to block of code
//       for (y = 0; y < w; y++) {
//         if (bitmatrix_element[index]) {
//           dptr = bdptr + sindex + y*packetsize;
//           cptr = bcptr + sindex + y*packetsize;
//           if (!pstarted) {
//             galois_region_xor(cptr, dptr, cptr, packetsize);
//             jerasure_total_xor_bytes += packetsize;
//             pstarted = 1;
//           } else {
//             galois_region_xor(cptr, dptr, cptr, packetsize);
